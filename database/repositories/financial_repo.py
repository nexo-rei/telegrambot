...
BUG FIXES:
  ...
  - Missing `or_` import from sqlalchemy (used in get_deposit_by_reference)
  - create_deposit signature used Optional[str] without the import
"""
...
from sqlalchemy import select, func, desc, update, exc, or_
...
class FinancialRepository(BaseRepository[Transaction]):
    ...
    async def create_deposit(
        ...
        fee: Decimal,
        external_ref: str,
        payment_ref: Optional[str] = None,  # BUG FIX: Optional was undefined before fix
    ) -> Transaction:
    ...
    async def get_deposit_by_reference(self, reference_str: str) -> Transaction | None:
        ...
        try:
            # BUG FIX: or_ was used but never imported. Added to imports above.
            ...
            logger.error(
                f"Failed deposit reference mapping lookup for reference {reference_str}: {error}"
            )
            raise error
    ...
    async def approved_withdrawals(self) -> Sequence[Transaction]:
        ...
        return await self.filter(
            type=TransactionType.WITHDRAW, status=TransactionStatus.PROCESSING
        )
    ...
    async def rejected_withdrawals(self) -> Sequence[Transaction]:
        ...
        return await self.filter(
            type=TransactionType.WITHDRAW, status=TransactionStatus.CANCELLED
        )
    ...
    async def transaction_history(self, limit: int = 50, offset: int = 0) -> Sequence[Transaction]:
        ...
        try:
            ...
            logger.error(
                f"Failed transaction systemic ledger history extraction sequence: {error}"
            )
            raise error
    ...
    async def latest_transactions(self, limit_count: int = 10) -> Sequence[Transaction]:
        ...
        try:
            ...
            logger.error(
                f"Failed processing top ledger slice retrieval limit {limit_count}: {error}"
            )
            raise error
    ...
    async def user_transactions(
        ...
        try:
            ...
            logger.error(
                f"Failed historical ledger extraction for target user identity node {user_id}: {error}"
            )
            raise error
    ...
    async def _get_wallet_locked(self, wallet_id: int) -> Wallet:
        ...
        if not wallet:
            raise ValueError(
                f"Target system critical wallet instance missing on allocation index lookup: {wallet_id}"
            )
        if wallet.is_locked:
            raise RuntimeError(
                f"Administrative lockdown operational barrier active on target wallet index: {wallet_id}"
            )
        return wallet

    async def credit_wallet(
        self, wallet_id: int, amount: Decimal, field_target: str = "available_balance"
    ) -> Wallet:
    ...
    async def debit_wallet(
        ...
        try:
            ...
            if current_val < amount:
                raise ValueError(
                    f"Insufficient funds processing transactional deduction on field {field_target}. "
                    f"Available: {current_val}, Requested: {amount}"
                )
    ...
    async def freeze_balance(self, wallet_id: int, amount: Decimal) -> Wallet:
        ...
        try:
            ...
            if wallet.available_balance < amount:
                raise ValueError(
                    "Insufficient liquid balances matching requested freezing containment constraints."
                )
    ...
    async def unfreeze_balance(self, wallet_id: int, amount: Decimal) -> Wallet:
        ...
        try:
            ...
            if wallet.frozen_balance < amount:
                raise ValueError(
                    "Requested asset extraction size exceeds current isolated frozen allocation boundaries."
                )
    ...
    async def transfer_between_balances(
        ...
        try:
            ...
            if source_val < amount:
                raise ValueError(
                    f"Insufficient asset balances inside source location structural element {source_field}."
                )
            ...
            logger.error(
                f"Failed atomic partition relocation sequence on internal wallet index {wallet_id}: {error}"
            )
            await self.session.rollback()
            raise error
    ...
    async def create_investment(self, payload: dict[str, Any]) -> Investment:
        ...
        try:
            ...
            logger.error(
                f"Failed to instantiate active investment asset record tracking nodes: {error}"
            )
            await self.session.rollback()
            raise error
    ...
    async def active_investments(self) -> Sequence[Investment]:
        ...
        try:
            ...
            logger.error(
                f"Failed compilation tracking listing on active capital allocations: {error}"
            )
            raise error
    ...
    async def completed_investments(self) -> Sequence[Investment]:
        ...
        try:
            ...
            logger.error(
                f"Failed compilation tracking listing on finalized capital allocations: {error}"
            )
            raise error
    ...
    async def calculate_daily_profit(self, investment_id: int) -> Decimal:
        ...
        try:
            ...
            logger.error(
                f"Failed analytical daily yield estimation sequence on index {investment_id}: {error}"
            )
            raise error
    ...
    async def mature_investments(self) -> int:
        ...
        try:
            ...
            logger.error(
                f"Failed systematic automated investment tracking maturity sweep: {error}"
            )
            await self.session.rollback()
            raise error
    ...
    async def pay_referral_bonus(
        ...
        payload = {
            ...
            "description": (
                f"Level {level} Affiliate Commission allocation from partner tracking index: {from_user_id}"
            ),
    ...
    async def referral_reward_history(self, user_id: int) -> Sequence[Transaction]:
        ...
        try:
            ...
            logger.error(
                f"Failed affiliate reward ledger selection history for identifier {user_id}: {error}"
            )
            raise error
    ...
    async def total_referral_paid(self) -> Decimal:
        ...
        try:
            ...
            logger.error(
                f"Failed systemic absolute affiliate reward compilation compute: {error}"
            )
            raise error
    ...
    async def platform_balance(self) -> Decimal:
        ...
        try:
            statement = select(
                func.sum(
                    Wallet.available_balance + Wallet.frozen_balance + Wallet.investment_balance
                )
            )
            result = await self.session.execute(statement)
            return Decimal(result.scalar() or "0.00")
